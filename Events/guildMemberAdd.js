...
module.exports = async(Bumper, member) => { // GuildMemberAdd Event.
  ...
  if(message === null){ msg = `Welcome To The Server ${member.user.tag}`; };
  if(message) { msg = `Welcome! ${member.user} ${message}`; }
  
  let back = await Quick.fetch(`welcomeIMG_${member.guild.id}`)
  if(back === null){ backk = "./assets/e76.jpg" };
  ...
  ctx.fillText('Welcome to the server,', canvas.width / 3.2,canvas.height /2.5)
