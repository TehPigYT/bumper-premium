...
if(!channel)return //console.log("No Partner Channel")
if(channel) {
...
if(!description)return //console.log("No Desc")
  
let invite = await Quick.fetch(`invite_${serverID}`);
if(!invite) {
  return console.log("No INV")
};
...
partnerEmbed.setURL(`https://bumper-premium.glitch.me/server/${guild.id}`)
.setDescription(`
      Owner **-** ${guild.owner}
